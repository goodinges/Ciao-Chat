...
int Configuration::Init(const char *filename) {
...
#ifdef ANDROID
	const char * defaultSendAnxbFileDir = "/sdcard/testvideos";
#else
	...
	m_Entries[index++].initIntEntry("LimitReceiveBitrate", 750,
			"limit receive bitrate to bitrate_kbps");
	m_Entries[index++].initIntEntry("LimitSendBitrate", 750,
			"limit send bitrate to bitrate_kbps (ignore higher values given by SetBitrate command)");
	m_Entries[index++].initIntEntry("ForceSendFramerate", 0,
			"Force certain send framerate instead of following the framerate set by SetFramerate command");
	m_Entries[index++].initIntEntry("PayloadFormat", 1,
			"Use either rtp packets (value 0) or annex-B frames (value 1)");
	m_Entries[index++].initIntEntry("TogglePayloadFormat", 1,
