...
//------------------------------
//SKYPE PATCH
void hash_and_hex(const unsigned char* buffer, int buffer_len, char* hash_hex, int hash_hex_len) 
{
    Sha buffer_sha;
    ...
    if (!buffer || !buffer_len || !hash_hex || !hash_hex_len) 
        return;
    
    InitSha(&buffer_sha);
    ShaUpdate(&buffer_sha, buffer, buffer_len);
    ShaFinal(&buffer_sha, digest);                                            
    ptr = &digest[0];    
    
    for (count = 0; count < hash_hex_len - 1; ptr++) {
        hash_hex[count++] = hex[*ptr >> 4];
        hash_hex[count++] = hex[*ptr & 0xf];
    }           
    hash_hex[hash_hex_len - 1] = '\0';
}

#ifdef SESSION_CERTS

void CyaSSL_get_peer_certificate_hash(SSL* ssl, char* peercert_hex, int peercert_hex_len)
{        
    if (!ssl->session.chain.count || !peercert_hex || !peercert_hex_len) 
        return;
    
    hash_and_hex(ssl->session.chain.certs[ssl->session.chain.count-1].buffer, 
        ssl->session.chain.certs[ssl->session.chain.count-1].length, peercert_hex, peercert_hex_len);   
}

#endif /* SESSION_CERTS */

void CyaSSL_get_certificate_hash(const char* cert_buf, long cert_buf_len, char* cert_hex, int cert_hex_len)
{ 
    EncryptedInfo info;
    buffer der;   
    
    if (!cert_buf || !cert_buf_len || !cert_hex || !cert_hex_len) 
        return;
    
    info.set = 0;
    der.buffer = 0;
    
    if (PemToDer(cert_buf, cert_buf_len, CERT_TYPE, &der, 0, &info) >= 0)
        hash_and_hex(der.buffer, der.length, cert_hex, cert_hex_len);    
    
    XFREE(der.buffer, 0, DYNAMIC_TYPE_CERT);
}
//SKYPE PATCH
//------------------------------
