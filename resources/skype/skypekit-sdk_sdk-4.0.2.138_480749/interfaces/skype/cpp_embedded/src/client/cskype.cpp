...
{
        String accname;
        if (GetDefaultAccountName(accname) && GetAccount(accname, activeAccount)) {
...
void CSkype::OnApp2AppStreamListChange(const String& appname, const APP2APP_STREAMS& listType, const List_String& streams, const Sid::List_uint &/*receivedSizes*/)
{
...
void CSkype::OnAvailableVideoDeviceListChange()
{
    printf("OnAvailableVideoDeviceListChange()\n");
    Sid::List_String name, path;
    uint count;
    GetAvailableVideoDevices(name,path,count);
...
void CSkype::OnH264Activated()
{
    printf("OnH264Activated\n");
}
