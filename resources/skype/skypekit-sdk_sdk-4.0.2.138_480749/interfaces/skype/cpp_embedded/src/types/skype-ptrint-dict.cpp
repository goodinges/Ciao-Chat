...
/** \cond INTERNAL */
class SEPtrIntDict::Element
{
public:
        Element(const int k, const void* v);
...
SEPtrIntDict::Element::Element(const int k, const void* v)
                :next(0),
                key(k),
                value(v)
{
...
void SEPtrIntDict::insert(const int key,  const void* new_value)
{
        detach();
...
const void*& SEPtrIntDict::find(const int key, unsigned int offset) const
{
...
{
        const_cast<SEPtrIntDict&>(dict).d_ref();
